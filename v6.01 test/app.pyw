...
def sufixo_valido(sku):
    ...
    return (
        ...
        not any(sku.endswith(suf) for suf in ['-F', '-P', '-V'])  # SKU sem sufixo vai para Loja 1
    )
...
def init_ean_database():
    ...
    try:
        ...
        conn.close()
# CHAMADA FORA DA FUNÇÃO (IMPORTANTE!)
init_ean_database()




def process_ean_text(text_input, user=None):
    """Processa texto com EANs e SKUs, rejeitando duplicatas no lote e validando sufixos."""
    results = {
        'success_count': 0,
        'error_count': 0,
        'errors': [],
    ...
    if not text_input or not text_input.strip():
        results['errors'].append("Texto de entrada vazio")
        return results
    ...
    # Passo 1: Verificar duplicatas no lote
    ...
    for line in lines:
        ...
        if len(parts) == 2 and parts[0].strip():
            skus.append(parts[0].strip().upper())

    sku_counts = Counter(skus)
    duplicates = [sku for sku, count in sku_counts.items() if count > 1]
    if duplicates:
        msg = f"Erro: SKUs duplicados encontrados no lote: {', '.join(duplicates)}. Nenhum item será processado."
        results['errors'].append(msg)
        results['error_count'] = len(duplicates)
        for sku in duplicates:
            log_ean_error(cursor, f"SKU duplicado: {sku}", "duplicado_lote", current_time)
        conn.commit()
        conn.close()
        logger.info(f"Processamento abortado devido a SKUs duplicados: {duplicates}")
        return results

    # Passo 2: Carregar pares SKU-EAN existentes
    existing_sku_ean = set()
    for row in cursor.execute("SELECT sku, ean FROM ean_data"):
        existing_sku_ean.add((row['sku'], row['ean']))
    ...
    try:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            parts = line.split('\t')
            if len(parts) != 2:
                msg = f"Linha {line_num}: Formato inválido. Use SKU[TAB]EAN"
                results['errors'].append(msg)
                results['error_count'] += 1
                log_ean_error(cursor, msg, "formato_invalido", current_time)
                continue

            sku, ean = parts[0].strip().upper(), parts[1].strip()
            if not sku or not ean:
                msg = f"Linha {line_num}: SKU ou EAN vazio"
                results['errors'].append(msg)
                results['error_count'] += 1
                log_ean_error(cursor, msg, "campo_vazio", current_time)
                continue

            # Validar formato do SKU
            if not sufixo_valido(sku):
                    msg = f"Linha {line_num}: Sufixo inválido em SKU '{sku}'. Sufixos permitidos: -999, -100, -130, -175, -VF, -F, -P, -V ou nenhum sufixo"
                    results['errors'].append(msg)
                    results['error_count'] += 1
                    log_ean_error(cursor, msg, "sufixo_invalido", current_time)
                    continue

            # Verificar duplicatas de EAN no lote
            if ean in eans_vistos:
                msg = f"Erro de Duplicidade: O EAN '{ean}' está duplicado neste lote."
                results['errors'].append(msg)
                results['error_count'] += 1
                log_ean_error(cursor, msg, "duplicado_lote", current_time)
                continue
            eans_vistos.add(ean)

            # Verificar se já existe no banco
            if (sku, ean) in existing_sku_ean:
                results['duplicates'].append(f"SKU: {sku}, EAN: {ean}")
                continue

            loja_id = classificar_loja_por_sku(sku)
            if loja_id is None:
                msg = f"Linha {line_num}: Loja não identificada para SKU '{sku}'"
                results['errors'].append(msg)
                results['error_count'] += 1
                log_ean_error(cursor, msg, "loja_invalida", current_time)
                continue

            valid_entries.append((sku, ean, current_time, user))
            valid_loja_status.append((loja_id,))

        # Passo 4: Inserir entradas válidas em lote
        if valid_entries:
            cursor.executemany("""
                INSERT INTO ean_data (sku, ean, created_date, created_by)
                VALUES (?, ?, ?, ?)
            """, valid_entries)

            cursor.execute("SELECT id, sku FROM ean_data WHERE created_date = ?", (current_time,))
            new_ids = {row['sku']: row['id'] for row in cursor.fetchall()}

            loja_status_entries = [(new_ids[sku], loja_id) for (sku, _, _, _), (loja_id,) in zip(valid_entries, valid_loja_status) if sku in new_ids]
            if loja_status_entries:
                cursor.executemany("""
                    INSERT INTO ean_loja_status (ean_data_id, loja_id)
                    VALUES (?, ?)
                """, loja_status_entries)

            results['success_count'] = len(valid_entries)
            logger.info(f"Processamento EAN concluído: {results['success_count']} sucessos, {results['error_count']} erros")

        conn.commit()

    except sqlite3.Error as e:
        conn.rollback()
        results['errors'].append(f"Erro interno no banco de dados: {str(e)}")
        results['error_count'] += 1
        logger.error(f"Erro durante processamento EAN: {str(e)}")
    except Exception as e:
        conn.rollback()
        results['errors'].append(f"Erro interno: {str(e)}")
        results['error_count'] += 1
        logger.error(f"Erro geral durante processamento EAN: {str(e)}")
    finally:
        conn.close()

    return results

def get_eans_by_loja(loja_id, sku_filter=None):
    """Retorna todos os EANs/SKUs para uma loja específica."""
    conn = get_ean_db()
    cursor = conn.cursor()
...
def update_ean_status(ean_data_id, loja_id, checked, user=None):
    """Atualiza o status de verificação de um EAN para uma loja."""
    conn = get_ean_db()
    cursor = conn.cursor()
    
    try:
        current_time = datetime.now().isoformat() if checked else None
        
        cursor.execute("""
            UPDATE ean_loja_status 
            SET checked = ?, checked_date = ?, checked_by = ?
            WHERE ean_data_id = ? AND loja_id = ?
        """, (1 if checked else 0, current_time, user, ean_data_id, loja_id))
        ...
        logger.error(f"Erro ao atualizar status EAN: {str(e)}")
        return False
    finally:
        conn.close()

def get_ean_statistics():
    """Retorna estatísticas gerais do sistema EAN."""
    conn = get_ean_db()
    cursor = conn.cursor()
    ...
    try:
        ...
        # Total de EANs cadastrados
        cursor.execute("SELECT COUNT(*) FROM ean_data")
        stats['total_eans'] = cursor.fetchone()[0]
        
        # Total por loja
        for loja_id in range(1, 5):
            cursor.execute("""
                SELECT COUNT(*) FROM ean_loja_status 
                WHERE loja_id = ?
            """, (loja_id,))
            stats[f'total_loja_{loja_id}'] = cursor.fetchone()[0]
            
            # Verificados por loja
            cursor.execute("""
                SELECT COUNT(*) FROM ean_loja_status 
                WHERE loja_id = ? AND checked = 1
            """, (loja_id,))
            stats[f'checked_loja_{loja_id}'] = cursor.fetchone()[0]
        
        # Total de erros não resolvidos
        ...
        logger.error(f"Erro ao obter estatísticas EAN: {str(e)}")
        return {}
    finally:
        conn.close()

def get_ean_errors():
    """Retorna lista de erros de cadastro não resolvidos."""
    conn = get_ean_db()
    cursor = conn.cursor()
    ...
    try:
        ...
        logger.error(f"Erro ao buscar erros EAN: {str(e)}")
        return []
    finally:
        conn.close()

def resolve_ean_error(error_id):
    ...
    try:
        ...
        logger.error(f"Erro ao resolver erro EAN {error_id}: {str(e)}")
        return False
    finally:
        conn.close()
...
@login_required
def consulta_ean():
    # Verifica se o usuário logado tem a permissão correta
...
def ean_page():
    ...
    try:
        # Serve o arquivo ean.html existente
        with open('ean.html', 'r', encoding='utf-8') as f:
            content = f.read()
        return content
    except FileNotFoundError:
...
def get_all_eans():
    try:
        conn = sqlite3.connect('ean_control.db')
        cursor = conn.cursor()

        cursor.execute("SELECT sku, ean, created_by FROM ean_data")
        rows = cursor.fetchall()
        conn.close()
        ...
        for sku, ean, criado_por in rows:
            if sku.endswith('-F'):
                loja = 'loja2'
            elif sku.endswith('-P'):
                loja = 'loja3'
            ...
            lojas[loja].append({
                'subloja': 'mercado',  # padrão, pode ser ajustado depois
                'sku': sku,
                'ean': ean
            })
...
def processar_eans():
    ...
    for linha in linhas:
        ...
        # REGRAS DE SUFIXO:
        # Aceita apenas:
        #   - sem sufixo (ex: PCDA001)
        #   - ou com -F, -P, -V no final
        # Rejeita se terminar com F, P ou V sem hífen (ex: PCDA001P)
        # Sufixos válidos para qualquer loja
        sufixos_validos = ['-F', '-P', '-V', '-130', '-999', '-100', '-175','-350', '-VF']

        tem_sufixo_valido = any(sku.endswith(suf) for suf in sufixos_validos)
        sem_sufixo = not '-' in sku  # Ex: PCDA001

        if not (tem_sufixo_valido or sem_sufixo):
        ...
        try:
            ...
            loja_id_map = {'mercado': 1, 'shopee': 2, 'shein': 3, 'magalu': 4, 'site': 5}
            for idx, nome in enumerate(['mercado', 'shopee', 'shein', 'magalu', 'site']):
                valor = partes[idx + 2].strip().lower() if len(partes) > idx + 2 else ''
                checked = 1 if valor in ['ok', 'x', '✓', '✔', '1'] else 0
                cursor.execute("""
                    INSERT INTO ean_loja_status (ean_data_id, loja_id, checked, checked_date, checked_by)
                    VALUES (?, ?, ?, ?, ?)
                """, (ean_id, loja_id_map[nome], checked, now if checked else None, 'upload' if checked else None))
...
def get_loja_eans(loja_id):
    ...
    try:
        ...
        eans = get_eans_by_loja(loja_id, sku_filter)
...
def update_status():
    ...
    try:
        ...
        success = update_ean_status(ean_data_id, loja_id, checked, user)
...
@app.route('/api/ean/statistics')
@login_required
def get_statistics():
    """Retorna estatísticas do sistema EAN."""
    try:
        stats = get_ean_statistics()
...
@app.route('/api/ean/errors')
@login_required
def get_errors():
    """Retorna lista de erros não resolvidos."""
    try:
        errors = get_ean_errors()
...
def resolve_ean_error_route(error_id):
    """Marca um erro como resolvido."""
    try:
        success = resolve_ean_error(error_id)
