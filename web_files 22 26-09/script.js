...
const defaultPermissions = {
    ...
    processadorEANs: { visualizar: false, editar: false, processar: false } 
...
function loadPermissionModules() {
    const modules = [
        ...
        // *** NOVA LINHA ADICIONADA AQUI ***
        { key: 'processadorEANs', label: 'Processador de EANs', actions: ['visualizar', 'editar', 'processar'] },
 ...
 * VERSÃO CORRIGIDA: Prioriza a verificação do sufixo '-350' para SKUs 'TP'.
...
function calcularPeso(sku) {
    ...
    // --- LÓGICA DE PESO PARA LONAS (TP) ---
    // 1. Verifica se é um SKU de lona (começa com "TP").
    if (skuUpper.startsWith('TP')) {
        // 2. Se for lona, verifica se contém o marcador de peso especial "-350".
        if (skuUpper.includes('-350')) {
            return '3,900'; // Lona pesada
        }
        // 3. Se for lona mas não tiver o marcador, usa o peso padrão para lonas.
        return '1,500'; // Lona padrão
    }

    // --- LÓGICA PARA OUTROS PREFIXOS ---
    // Extrai a sequência inicial de letras do SKU.
    const match = skuUpper.match(/^[A-Z]+/);
    const prefixoCompleto = match ? match[0] : null;

    if (!prefixoCompleto) return 'N/A';

    // Verifica os outros prefixos conhecidos.
    if (prefixoCompleto.startsWith('PR')) return '0,300';
    if (prefixoCompleto.startsWith('PV')) return '0,500';
    if (prefixoCompleto.startsWith('PH')) return '0,200';
    if (prefixoCompleto.startsWith('KC')) return '1,500';
    if (prefixoCompleto.startsWith('KD')) return '1,500';
    if (prefixoCompleto.startsWith('PC')) return '1,000';
    if (prefixoCompleto.startsWith('VC')) return '1,200';
    if (prefixoCompleto.startsWith('CL')) return '0,700';
    if (prefixoCompleto.startsWith('RV')) return '0,800';
    if (prefixoCompleto.startsWith('FF')) return '0,500';

    // Se nenhuma das regras corresponder, retorna 'N/A'.
    return 'N/A';
}
 ...
 * VERSÃO FINAL: Controla checkboxes e botões com base nas novas permissões.
 */
    ...
    if (itensFiltrados.length > 0) {
                        ...
                        <tr id="${item.id}" class="border-t hover:bg-gray-50">
                            <td class="p-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button onclick="copiarParaClipboard('${item.sku}')" class="copy-btn" title="Copiar SKU"><i class="fas fa-copy"></i></button>
                                </div>
                            </td>
                            <td class="p-3">
                                <div class="flex items-center justify-between">
                                    <span class="font-mono">${item.ean}</span>
                                    <button onclick="copiarParaClipboard('${item.ean}')" class="copy-btn" title="Copiar EAN"><i class="fas fa-copy"></i></button>
                                </div>
                            </td>
                            <td class="p-3">
                                <div class="flex items-center justify-between">
                                    <span class="font-medium">${item.peso || 'N/A'}</span>
                                    <button onclick="copiarParaClipboard('${item.peso || ''}')" class="copy-btn" title="Copiar Peso"><i class="fas fa-copy"></i></button>
                                </div>
                            </td>
                            <td class="p-3">
                                <div class="flex items-center justify-between">
                                    <span class="font-mono">${item.ncm || 'N/A'}</span>
                                    <button onclick="copiarParaClipboard('${item.ncm || ''}')" class="copy-btn" title="Copiar NCM"><i class="fas fa-copy"></i></button>
                                </div>
                            </td>
        ...
        resultContainer.innerHTML = `
            <style>
                ...
                .copy-btn:hover {
                    color: #4F46E5; /* Cor índigo do Tailwind */
                }
            </style>
...
function abrirModalEdicaoEAN(itemId) {
    ...
    // --- ALTERAÇÃO APLICADA AQUI ---
    // Preenche todos os quatro campos do modal
...
function salvarEdicaoEAN() {
    ...
    const novoPeso = document.getElementById('edit-peso-input').value.trim();
    const novoNcm = document.getElementById('edit-ncm-input').value.trim();
    ...
    // Validações de duplicidade (código inalterado)
    ...
    // Atualiza o objeto do item com todos os novos valores
    const item = listaEANs[itemIndex];
    item.sku = novoSku;
    item.ean = novoEanLimpo;
    item.peso = novoPeso; // Salva o peso editado
    item.ncm = novoNcm;   // Salva o NCM editado
    
    saveData();
    logAction(`Item EAN editado: SKU '${itemOriginal.sku}' -> '${novoSku}', EAN '${itemOriginal.ean}' -> '${novoEanLimpo}', Peso '${itemOriginal.peso}' -> '${novoPeso}', NCM '${itemOriginal.ncm}' -> '${novoNcm}'.`);
 ...
 */
function gerarRelatorioEANsPDF() {
    // 1. CONTROLE DE PERMISSÃO ATUALIZADO
    // A função agora verifica se o usuário tem a permissão para 'processar'.
    if (!hasPermission('processadorEANs', 'processar')) {
        showToast('Você não tem permissão para gerar este relatório.', 'error');
        return; // Interrompe a execução se não tiver permissão.
    }

    // 2. Coleta dos valores dos filtros (código inalterado)
    ...
    const itensFiltrados = listaEANs.filter(item => {
        ...
        if (!pertenceALoja) return false;
        if (skuPrefixo && !item.sku.startsWith(skuPrefixo)) return false;

        if (marketplaceFiltro === 'Allmarket' && statusFiltro !== 'todos') {
            const dadosDaLoja = item.lojas?.[lojaId];
            if (!dadosDaLoja) return statusFiltro === 'nao_cadastrados';
            const cadastradoEmAlgum = marketplacesParaVerificar.some(mp => dadosDaLoja.marketplaces?.[mp]?.marcado);
            if (statusFiltro === 'cadastrados') return cadastradoEmAlgum;
            if (statusFiltro === 'nao_cadastrados') return !cadastradoEmAlgum;
        }
        else if (marketplaceFiltro !== 'Allmarket' && statusFiltro !== 'todos') {
            const isCadastrado = item.lojas?.[lojaId]?.marketplaces?.[marketplaceFiltro]?.marcado || false;
    ...
    const data = new Date();
    const nomeArquivo = `Relatorio_EANs_${lojaConfig.nome.replace(' ', '')}_${data.toLocaleDateString('pt-BR').replace(/\//g, '-')}.pdf`;
    doc.save(nomeArquivo);

    logAction(`Relatório de EANs em PDF gerado para Loja: ${lojaConfig.nome}, Marketplace: ${marketplaceFiltro}, SKU: ${skuPrefixo || 'Todos'}.`);
