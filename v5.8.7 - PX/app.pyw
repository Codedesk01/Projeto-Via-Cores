            ...
            try:
                ...
                if action == 'bulk_add':
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (sku, 'entrada', quantity, current_date, caixa_to_log, classificacao_str, observacao_outros))
                    ...
                    cursor.execute("""
                        INSERT INTO transactions (sku, transaction_type, quantity, date, caixa, classificacao, observacao)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (sku, 'saida', quantity, current_date, current_caixa, classificacao_str, observacao_outros))
            ...
            for termo in termos:
                ...
                if is_complete_sku:
                    if current_user.role == 'impressao':
                        ...
                        conditions.append("(codigo = ? OR codigo LIKE ?)")
                        params.append(termo)
                        params.append(f"{termo}%")
                else:
                    # Busca ampla para termos genéricos
                    conditions.append("codigo LIKE ?")
                    params.append(f"%{termo}%")
                ...
                for row in cursor.fetchall():
                    if row['codigo'] not in [item['codigo'] for item in items]:  # Evita duplicatas
                        ...
                        print(f'[Server] Encontrado SKU: {row["codigo"]} (caixa: {row["caixa"]}) para termo {termo} (role: {current_user.role})')

        print(f'[Server] Busca por SKU: {input_raw}, Caixa: {caixa_filter}, Resultados: {len(items)} para {current_user.username} (role: {current_user.role})')
...
def all_sku_transactions():
    ...
    # Adiciona lista de classificações para cada SKU
